...
In \ttt{wishes} werden als eine \ttt{set} von Integers die internen Indizes der gewünschten 
Obstsorten, also der Elemente der Menge $W$, gespeichert.
Analog werden in \ttt{result}, also ebenfalls in eine \ttt{set} von Integers, die Indizes der gewünschten 
...
Die jeweiligen 1--en in \ttt{matrix} werden mithilfe der eingebauten Funktion \ttt{\_Find\_next()}
gefunden.\\
...
jeden Knotens in \ttt{G} zu überprüfen: Es wird geprüft, ob zwei Nachbarn dieselbe Kardinalität besitzen.
Wir nehmen dazu die Methode \ttt{getFirstNeighbor()} der Klass \ttt{Graph} und vergleichen die
